...
def envelope(contract, source, destination, payload, *, mid='MSG-1', idem='IDEM-1'):
    ...
    return {
      ...
      'payload_checksum_sha256':integration.payload_checksum(payload),
...
def insert_release(c, rid='REL-1', ver='1', checksum='a'*64):
    ...
    values={
      ...
      'payload_checksum_sha256':'b'*64,'release_status':'ACADEMICALLY_READY','local_status':'STAGED',
...
def test_new_dispatch_targets(monkeypatch):
    ...
    expected={
      bridge.DELIVERY_ACK:'/api/integration/v1/scoremax/content-delivery-acks',
      bridge.INCIDENT:'/api/integration/v1/scoremax/content-incidents',
      bridge.WITHDRAWAL_ACK:'/api/integration/v1/scoremax/question-withdrawal-acks',
    }
    for contract,path in expected.items():
        url,got,direction=integration._dispatch_target(contract)
        assert got==path
        assert url=='https://powerhouse.example.test'+path
...
def ph_question_row(c, *, ph=True):
    ensure_question_table(c)
    owner='POWER_HOUSE' if ph else ''
    c.execute('''INSERT INTO questions(question_id,question,active,status,ph_projection_owner,ph_question_id,ph_question_version_id,ph_question_checksum_sha256,ph_release_id,ph_release_version,ph_release_checksum_sha256,ph_market_id,ph_programme_id,ph_subject_id,ph_chapter_id)
                 VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(
      'LOCAL-1','What is immunity?',1,'Approved',owner,'PH-Q-1' if ph else '','PH-QV-1' if ph else '',
      'd'*64 if ph else '','PH-REL-1' if ph else '','1' if ph else '','e'*64 if ph else '',
      'PK' if ph else '','FSC2' if ph else '','BIOLOGY' if ph else '','CH19' if ph else ''))
...
def test_ph_incident_exact_lineage_idempotent_and_privacy_minimised(tmp_path):
    ...
    env=json.loads(c.execute("SELECT envelope_json FROM integration_outbox WHERE contract_name=?",(bridge.INCIDENT,)).fetchone()['envelope_json'])
    p=env['payload']; qp=p['question']
    assert qp['question_id']=='PH-Q-1'
    assert qp['question_version_id']=='PH-QV-1'
    assert qp['question_checksum_sha256']=='d'*64
    assert qp['release_id']=='PH-REL-1' and qp['release_checksum_sha256']=='e'*64
    assert p['reporter_identity_included'] is False and p['student_pii_included'] is False
    assert p['release_authority_conferred'] is False
...
def withdrawal_payload(qid='PH-Q-1', item='ITEM-1', export='EXP-1'):
    row={
      'item_public_id':item,
      'withdrawal_queue_id':1,
      'question_public_id':qid,
      'question_version_fingerprint':'PH-QV-1',
      'hold_id':2,
      'defect_code':'SCOREMAX_LEARNER_REPORTED_CONTENT_INCIDENT',
      'reason':'Governed fail-close test',
      'requested_action':'STOP_FUTURE_DELIVERY_PRESERVE_HISTORICAL_ATTEMPTS',
      'historical_attempts_must_be_preserved':True,
      'item_sha256':'f'*64,
    }
...
def test_withdrawal_stops_future_delivery_and_preserves_history(tmp_path):
    ...
    before_attempts=c.execute('SELECT COUNT(*) n FROM attempts').fetchone()['n']
    before_answers=c.execute('SELECT COUNT(*) n FROM attempt_answers').fetchone()['n']
    ...
    q2=c.execute("SELECT active,status FROM questions WHERE id=?",(q['id'],)).fetchone()
    assert q2['active']==0 and q2['status']=='Withdrawn'
    assert c.execute('SELECT COUNT(*) n FROM attempts').fetchone()['n']==before_attempts
    assert c.execute('SELECT COUNT(*) n FROM attempt_answers').fetchone()['n']==before_answers
    out=c.execute("SELECT envelope_json FROM integration_outbox WHERE contract_name=?",(bridge.WITHDRAWAL_ACK,)).fetchone()
    ack=json.loads(out['envelope_json'])['payload']
    assert ack['items'][0]['state']=='WITHDRAWN'
    assert ack['items'][0]['historical_attempts_preserved'] is True
    assert ack['release_authority_conferred'] is False
    c.close()
...
def test_withdrawal_exact_export_replay_is_duplicate_without_mutation(tmp_path):
    c=conn(tmp_path); q=ph_question_row(c,ph=True)
    p=withdrawal_payload(); env=envelope(bridge.WITHDRAWAL,'POWER_HOUSE','SCOREMAX',p,mid='WD-1',idem='WD-IDEM-1')
    r1,s1=bridge.admit_withdrawal_envelope(c,env,env['payload_checksum_sha256'])
    r2,s2=bridge.admit_withdrawal_envelope(c,env,env['payload_checksum_sha256'])
    assert (s1,r1['status'])==(202,'ACCEPTED')
    assert (s2,r2['status'])==(200,'DUPLICATE')
...
def test_unknown_withdrawal_returns_not_present(tmp_path):
    c=conn(tmp_path); ensure_question_table(c)
    p=withdrawal_payload(qid='PH-Q-NOT-HERE',item='ITEM-X',export='EXP-X'); env=envelope(bridge.WITHDRAWAL,'POWER_HOUSE','SCOREMAX',p,mid='WD-X',idem='WD-X')
    rec,status=bridge.admit_withdrawal_envelope(c,env,env['payload_checksum_sha256'])
    assert status==202 and rec['status']=='ACCEPTED'
