...
ROOT=Path('/dev/shm/scoremax-emergency-return-qual')
...
def main():
    sm.init()
    # Central bridge can resolve governed Emergency Direct evidence from exact stored row.
    ...
    # Actual admin reject must atomically make local state inactive and queue one PH incident.
    ...
    # Invalid governed identity must block a fresh reject and roll local state back.
    raw2=make_csv(ph_id='',qid='BIO12-CH13-QUAL-002'); cl2,qdb2,bid2,digest2,ledger2=import_release(raw2)
    r=cl2.post(f'/admin/questions/{qdb2}/review',data={'_csrf_token':'qual-csrf','action':'reject','reason_code':'Other','note':'must fail'},follow_redirects=False)
    ...
    print('SCOREMAX_EMERGENCY_RETURN_BRIDGE_QUALIFICATION_PASS emergency_identity=true admin_reject_atomic=true invalid_identity_rollback=true outbox_exact=true integrity=ok')
