...
POLICY='SCOREMAX-PERSISTENT-GUARD-V2-PREPOSTIMPORT-20260913'
...
def qualify_database():
    ...
    try:
        ...
        if counts['coverage_packages']!=0: fail(f'commercial_catalogue_not_empty count={counts["coverage_packages"]}')
        ...
        if counts['questions']==0:
            ...
            return {'state':'preimport_qualified','policy':POLICY,'integrity':integrity,'fk':fk,'counts':counts,'priced_plans':priced,'reviewer_state':reviewers,'backup':str(BASELINE_BACKUP),'backup_sha256':sha,'backup_integrity':bi,'backup_fk':bfk}
...
def main():
    ...
    if SENTINEL.exists():
        ...
        ident=str(uuid.uuid4()); payload={'storage_id':ident,'purpose':'ScoreMax governed persistent storage','schema':2,'policy':POLICY}; tmp=SENTINEL.with_suffix('.tmp')
