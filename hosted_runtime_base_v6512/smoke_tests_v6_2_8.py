...
def main():
    ...
    ok('desktop student navigation has six persistent primary journeys plus a supporting account menu',all(x in base for x in ['>Home</a>','>Learn</a>','>My Plan</a>','>Practice</a>','>Exams</a>','>Progress</a>']) and 'student-account-menu' in base)
    ok('mobile uses reduced bottom navigation while the subject strip remains the only contextual study row',base.count('mobile-bottom-nav')>=1 and 'subject-quick-strip' in base and 'student-secondary-nav' not in base)
    ...
    ok('public Daily Spark is visible before login without a submission form','DAILY SPARK' in index and 'public_spark' in index and 'student_daily_spark_action' not in index)
    ok('programme options are direct Available or Coming Soon cards without category layers','public_programmes' in index and 'Coming Soon' in index and 'FOR SCHOOLS' not in index)
