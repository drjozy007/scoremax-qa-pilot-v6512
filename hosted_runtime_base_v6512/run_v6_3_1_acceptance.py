"""ScoreMax V6.3.1 Student UX V2 acceptance runner.

Preserves the 441 inherited capability checks, the 82 V6.3 mastery/application
checks, and adds the V6.3.1 student UX contract. Historical UI assertions that
were explicitly superseded by the accepted UX V2 decision are archived in
frozen_legacy_ux_assertions/ rather than silently discarded.
"""
...
def main():
    ...
    p=subprocess.run([sys.executable,str(ROOT/'scoremax_v6_3_simulator.py'),'--learners','1000','--fuzz','10000','--output',str(ROOT/'V6_3_1_SIMULATION_RESULTS_QUICK.json')],cwd=ROOT,text=True,capture_output=True)
    print('\n=== synthetic simulation ===\n'+p.stdout.rstrip())
    if p.returncode:
        print(p.stderr,file=sys.stderr); raise SystemExit(p.returncode)
    print(f'\nSCOREMAX V6.3.1 ACCEPTANCE PASS: {inherited} inherited + {v63} V6.3 + {ux} UX V2 = {inherited+v63+ux} deterministic checks + synthetic simulation.')
