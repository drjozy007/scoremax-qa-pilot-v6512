"""Backup/restore utility for ScoreMax V6.5.1 local internal-live SQLite database."""
from __future__ import annotations
import argparse, hashlib, json, shutil, sqlite3
from datetime import datetime
from pathlib import Path
BASE=Path(__file__).resolve().parent
DATA=BASE/'internal_live_data'; DB=DATA/'scoremax_v6_5_1_internal_live.db'; BACKUPS=DATA/'backups_v651'
...
def backup():
    if not DB.exists(): raise SystemExit('No V6.5.1 internal-live database exists yet.')
    BACKUPS.mkdir(parents=True,exist_ok=True); stamp=datetime.now().strftime('%Y%m%d_%H%M%S'); out=BACKUPS/f'scoremax_v6_5_1_{stamp}.db'
