...
def main():
    ...
    ok('primary navigation prioritises learning before support',all(x in desktop for x in ['Dashboard','Learn','My Plan','Exams','Progress','More']))
    ok('teacher discovery and messages remain supporting links under More','Find a Teacher' in base and 'Academic Messages' in base and "'more':" in (ROOT/'app.py').read_text())
    dashboard=(ROOT/'templates/student.html').read_text(); ok('dashboard is a compact command centre with progressive tabs','command-centre-grid' in dashboard and dashboard.count('data-tab-panel=')>=4)
    ok('dashboard keeps teacher support in the lower-priority activity panel',dashboard.index('RECENT TESTS')<dashboard.index('Find a Teacher'))
    ...
    ok('practice setup uses progressive disclosure tabs',all(x in setup for x in ['Quick start','Choose scope','Design my own']))
    ...
    detail=(ROOT/'templates/subject_detail.html').read_text(); ok('coming-soon subjects cannot start fake tests',"selected.availability=='LIVE'" in detail and 'Governed practice will open' in detail)
