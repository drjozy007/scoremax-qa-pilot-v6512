...
def recalc(env, message_suffix=''):
    ...
    for q in (env.get('payload') or {}).get('questions') or []:
        q['question_checksum_sha256']=integ._object_checksum(q,'question_checksum_sha256')
    for st in (env.get('payload') or {}).get('stimuli') or []:
...
def product_activate(c,env,reason='focused integration regression'):
    r=env['payload']['release']
    return integ.authorize_product_activation(c,r['release_id'],r['release_version'],r['package_checksum_sha256'],'TEST::V650',reason)
...
for t in ['integration_ph_content_releases','integration_ph_question_versions','integration_ph_blueprints','integration_outbox','integration_quarantine']:
    ...
    if n>=1: break
ok('release identity exposes additive V6.5 child while preserving compatibility marker',app.healthz()[0]['version']=='6.2.8.1' and is_compatible_descendant(app.healthz()[0]['release_version'],'6.5.3'))
...
receipt,status=integ.admit_content_envelope(c,base,base['payload_checksum_sha256']); product_activate(c,base)
...
ok('exact approved-content replay returns the original durable receipt',status2==200 and receipt2==receipt)
...
rr,ss=integ.admit_content_envelope(c,ed,ed['payload_checksum_sha256']); product_activate(c,ed); qdg=ed['payload']['questions'][0]
...
st={'stimulus_id':'STIM::opaque::A-01','stimulus_version_id':'STIMV::opaque::A-01::v1','stimulus_checksum_sha256':'b'*64,'stimulus_type':'TEXT','content':{'text':'A governed shared stimulus.'},'provenance':{'source_id':'fixture::stimulus','source_type':'TEST_FIXTURE','rights_status':'OWNED'}}
...
rr,ss=integ.admit_content_envelope(c,es,es['payload_checksum_sha256']); product_activate(c,es); qst=es['payload']['questions'][0]
...
rr,ss=integ.admit_content_envelope(c,em,em['payload_checksum_sha256']); product_activate(c,em); qin=em['payload']['questions'][0]
...
# 16-17 rubric-only constructed response fails closed until a governed human-marking path exists.
...
ok('rubric-only content is rejected before learner projection while runtime lacks governed human marking',ss==422 and not r)
ok('rubric-only rejection names the unsupported delivery capability',any(e.get('code')=='UNSUPPORTED_RUBRIC_ONLY_DELIVERY' for e in rr.get('errors') or []))
...
integ.admit_content_envelope(c,v1,v1['payload_checksum_sha256']); product_activate(c,v1); qpin=v1['payload']['questions'][0]; qr=c.execute('SELECT id FROM questions WHERE ph_question_id=?',(qpin['question_id'],)).fetchone(); qdb=int(qr['id'])
# create student
...
integ.admit_content_envelope(c,v2,v2['payload_checksum_sha256']); product_activate(c,v2)
...
withdraw=example('v1_1_0/PH_SM_APPROVED_CONTENT_V1_1_WITHDRAW.example.json'); wr=withdraw['payload']['release']; wr['release_id']=v2['payload']['release']['release_id']; wr['release_version']='PIN3'; wr['supersedes_release_version']='PIN2'; wr['market_id']=v2['payload']['release']['market_id']; wr['programme_id']=v2['payload']['release']['programme_id']; wr['subject_id']=v2['payload']['release']['subject_id']; wr['chapter_id']=v2['payload']['release']['chapter_id']; withdraw['idempotency_key']='release::withdraw::pin3'; withdraw['message_id']='msg::withdraw::pin3'; withdraw=recalc(withdraw)
...
ok('exact blueprint replay returns the original durable receipt',bs2==200 and br2==br)
...
ok('Power House blueprint projects into the existing ScoreMax runtime without inventing academic identity',bpr['projection_status']=='ACTIVE_RUNTIME' and bp['payload']['blueprint_id'] in bpr['immutable_payload_json'])
...
app.refund_payment_transaction(c,tx,100000,'Integration governed full refund'); integ.sync_growth_outbox(c,'6.5.0'); events2=[json.loads(r['envelope_json']) for r in c.execute("SELECT * FROM integration_outbox WHERE contract_name='SM_GE_PRODUCT_EVENT_V1'").fetchall()]
...
rrw_after=c.execute('SELECT * FROM referral_rewards WHERE id=?',(rrw['id'],)).fetchone()
ok('governed refund reverses direct and one-upstream reward authority',rrw_after['status']=='reversed' and rrw_after['override_status']=='reversed')
...
snap_scope=integ.canonical_json({'ph_market_id':qrow['ph_market_id'],'ph_programme_id':qrow['ph_programme_id'],'ph_subject_id':qrow['ph_subject_id'],'ph_chapter_id':qrow['ph_chapter_id']})
for i in range(10):
