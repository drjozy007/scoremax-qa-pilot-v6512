"""Backup/restore utility for ScoreMax V6.5.4 local internal-live SQLite database."""
from __future__ import annotations
import argparse, hashlib, json, shutil, sqlite3
from datetime import datetime
from pathlib import Path
BASE=Path(__file__).resolve().parent
DATA=BASE/'internal_live_data'; DB=DATA/'scoremax_v6_5_4_internal_live.db'; BACKUPS=DATA/'backups_v654'
...
def backup():
    if not DB.exists(): raise SystemExit('No V6.5.4 internal-live database exists yet.')
    BACKUPS.mkdir(parents=True,exist_ok=True)
    stamp=datetime.now().strftime('%Y%m%d_%H%M%S')
    out=BACKUPS/f'scoremax_v6_5_4_{stamp}.db'
