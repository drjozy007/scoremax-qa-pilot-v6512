...
const levels=[['Foundation','Build understanding'],['Exam Ready','Use it correctly'],['Advanced','Handle harder questions'],['Distinction','Reduce mistakes'],['Expert','Perform under pressure'],['Elite','Prove consistency']];
...
function cleanLeakedScriptText(){if(!document.body)return;const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const bad=[];while(walker.nextNode()){const n=walker.currentNode;const t=(n.nodeValue||'').trim();if(t.length>120&&(t.includes("document.querySelectorAll('a')")||t.includes("commercial-paywall")||t.includes("Recorded%20Learning")))bad.push(n);}bad.forEach(n=>n.remove());}
function apply(){cleanLeakedScriptText();const old=document.querySelector('.mastery-hero-card');if(!old){placeDailySpark();return !!document.getElementById('uxStudentMasteryStrip');}const stage=authoritativeStage(old);render(stage);cleanLegacy(old);placeDailySpark();cleanLeakedScriptText();return true;}
function boot(){apply();[0,80,250,700,1400].forEach(ms=>setTimeout(function(){apply();placeDailySpark();cleanLeakedScriptText();},ms));const obs=new MutationObserver(function(){const old=document.querySelector('.mastery-hero-card');if(old){const stage=authoritativeStage(old);render(stage);cleanLegacy(old);}placeDailySpark();cleanLeakedScriptText();});obs.observe(document.body,{childList:true,subtree:true});setTimeout(()=>obs.disconnect(),5000);}
