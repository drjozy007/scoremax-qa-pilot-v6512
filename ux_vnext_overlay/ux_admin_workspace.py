...
def _simplify_admin_navigation(path: Path) -> None:
    ...
    desktop='''{% elif session.get('role')=='admin' %}
        <a href="{{url_for('admin_dashboard')}}">Home</a><a href="{{url_for('admin_view_as')}}">View As</a><a href="{{url_for('admin_questions')}}">Questions</a><a href="/student/content-review/staged?batch=cross50">Cross50 Review</a><a href="{{url_for('admin_users')}}">Users</a><a href="{{url_for('admin_interests')}}">Interest</a><a href="{{url_for('admin_analytics')}}">Analytics</a><a href="{{url_for('admin_mastery_lab')}}">QA &amp; Mastery</a><a href="{{url_for('admin_mastery_rigor')}}">Mastery & Rigor</a><a href="{{url_for('admin_payments')}}">Payments</a><a href="{{url_for('admin_referrals')}}">Referrals</a><a href="{{url_for('admin_integration_health')}}">Integration</a><a href="{{url_for('admin_exams')}}">Exams</a><a href="{{url_for('logout')}}">Logout</a>
      {% else %}'''
  ...
  {% elif session.get('user_id') %}'''
    ...
    for token in ("url_for('admin_dashboard')","url_for('admin_view_as')","url_for('admin_questions')","url_for('admin_users')","url_for('admin_interests')","url_for('admin_mastery_lab')","url_for('admin_mastery_rigor')","url_for('admin_integration_health')",'>Logout</a>','admin-preview-bar',"url_for('admin_view_as_exit')"):
    ...
    for forbidden in ('Reviewer Workspace','Question Families','Governance Audit','Direct Intake','Power House Bridge','<details'):
...
<section class="ux-admin-home">
  <section class="card ux-admin-hero"><p class="eyebrow">SCOREMAX ADMIN</p><h1>Platform operations</h1><p>Manage accounts, demand, access, payments, product operations and system integration. Academic question review and academic release authority remain in Power House.</p><div class="ux-admin-actions"><a class="btn" href="{{url_for('admin_view_as')}}">View ScoreMax as…</a><a class="btn alt" href="{{url_for('admin_questions')}}">Inspect questions</a><a class="btn alt" href="/student/content-review/staged?batch=cross50">Review Cross50</a></div></section>
...
def apply_admin_workspace(root: Path) -> None:
    ...
    for token in ('SCOREMAX ADMIN','System boundary','Academic question review and academic release authority remain in Power House.',"url_for('admin_view_as')","url_for('admin_questions')","url_for('admin_interests')","url_for('admin_mastery_lab')","url_for('admin_integration_health')"):
