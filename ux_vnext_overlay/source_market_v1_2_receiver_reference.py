...
def _matching_surface_items(content,stimuli,matching_key):
    ...
    if not stim and content.get('stimulus_ref'):
        stim=stimuli.get(str(content.get('stimulus_ref')))
    raw=_learner_stimulus_text(stim)
    labels={}
    for line in raw.splitlines():
    ...
    if matching_declared:
        ...
        if matching is None:
            ...
            elif _matching_surface_items(content,stimuli,matching) is None:
                errors.append({'code':'MATCHING_SURFACE_UNRESOLVED','path':f'{path}.content','message':'Matching visible material cannot be compiled into deterministic left/right learner controls','retryable':False})
...
def _projection(q,stimuli):
    ...
    if matching_key:
        matching_surface=_matching_surface_items(content,stimuli,matching_key)
        if matching_surface is not None:
            answer_cfg['left_items']=matching_surface['left_items']
            answer_cfg['right_options']=matching_surface['right_options']
            answer_cfg['allow_duplicate_targets']=matching_surface['allow_duplicate_targets']
...
,line)
        ...
        if label in labels and labels[label]!=text:
            return None
        labels[label]=text
    required=set(matching_key.keys())|set(matching_key.values())
    if not required.issubset(set(labels)):
        return None
    left=[{'id':lid,'text':labels[lid]} for lid in matching_key.keys()]
    right_prefixes={re.match(r'^[A-Za-z_]+',str(v)).group(0).upper() for v in matching_key.values() if re.match(r'^[A-Za-z_]+',str(v))}
    right=[]
    for label,text in labels.items():
        if label in matching_key:
            continue
        m=re.match(r'^[A-Za-z_]+',label)
        prefix=m.group(0).upper() if m else ''
        if not right_prefixes or prefix in right_prefixes:
            right.append({'id':label,'text':text})
    if not set(matching_key.values()).issubset({x['id'] for x in right}):
        return None
    return {'left_items':left,'right_options':right,'allow_duplicate_targets':False,'visible_text':raw}
