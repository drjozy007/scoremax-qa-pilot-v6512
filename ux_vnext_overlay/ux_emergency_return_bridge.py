...
MARKER='SM-PH-EMERGENCY-RETURN-BRIDGE-20260916-CURRENT'
...
"""
    ...
    rows=c.execute('''SELECT r.*,b.batch_code,b.source_system,b.source_prompt_pack_id,b.source_prompt_pack_version,b.payload_checksum,
...
def queue_reported_question_incident(c, question, feedback_code, category, severity, description, context):
    \"\"\"Queue one privacy-minimised incident for native PH lineage or governed Emergency Direct provenance.\"\"\"
    if not question: return ''
    keys=set(question.keys()) if hasattr(question,'keys') else set(question)
    def val(k): return question[k] if k in keys else None
    code=str(feedback_code or '').strip()
