...
def main():
    ...
    if not keys: raise SystemExit('V669B_ENV_PAYLOAD_MISSING')
    encoded=''.join(os.environ[k] for k in keys)
    raw=base64.b64decode(encoded,validate=True)
    if sha(raw)!=PAYLOAD_SHA: raise SystemExit('V669B_ENV_PAYLOAD_SHA_MISMATCH')
    ...
    print('V669B_HOSTED_RUNTIME_VERIFIED', 'payload_sha256='+PAYLOAD_SHA, 'source_candidate_sha256='+SOURCE_SHA, 'hero_sha256='+HERO_SHA, 'files='+str(len(files)), 'status=DEPENDENT_CANDIDATE_NOT_FROZEN')
